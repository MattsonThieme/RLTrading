...
# Can try getting to a positive episode value with the standard trade-based method and then moving to an episode based optimization
...
class Agent(object):
    ...
    def __init__(self, policy, state_size):
        ...
        self.EPS_START = 0.9
        self.EPS_END = 0.005
        self.EPS_DECAY = 400000  # Increasing in the hopes that it will help the model learn more about short term opportunities - used to be 10k
    ...
    def report(self, ask, scale, step, total, last):
        if (len(self.gains) >= 20) or (last):
            ...
            print("     Episode total: ${}\n\n\n".format(round(self.episode_profit,2)))
    ...
    def take_action(self, state, properties, last_ask, ask, next_env, next_ask):
        ...
        self.episode_memory.append([state, properties, action, reward, target, next_reward])
    ...
    def reward_calc(self, action, asset_status, investment, bought, hold_time, fee, ask):
        ...
        if self.asset_status == 1:
            ...
            if action == 2:
                ...
                reward = self.profit(ask)
                self.episode_profit += self.investment_scale*reward
        ...
        for j in range(self.env.num_episodes):
            ...
            for e, episode in enumerate(self.env.train_env):
                ...
                for i, state in enumerate(episode[:episode.shape[0]-1]):
                    ...
                    if i%(int(episode.shape[0]/10)) == 0:
                        print("{}/{}".format(i, episode.shape[0]))
